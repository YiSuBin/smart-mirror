var FUNCTIONSERVICE = {
  ...
  zoomin : function($scope,GeolocationService,MapService) {
                console.debug("Zoooooooomiiiiiiiinnn!!!");
                $scope.map = MapService.zoomIn();
          if(responsiveVoice.voiceSupport()) {
             responsiveVoice.speak("지도 확대","Korean Female");
        }
},
  zoomout : function($scope,GeolocationService,MapService) {
                console.debug("Zoooooooomouuuuuuutttt!!!");
                $scope.map = MapService.zoomOut();
          if(responsiveVoice.voiceSupport()) {
             responsiveVoice.speak("지도 축소","Korean Female");
        }
},
  zoomreset : function($scope,GeolocationService,MapService) {
                console.debug("Zoomreeeesssseeeettttt!!!");
                $scope.map = MapService.reset();
          if(responsiveVoice.voiceSupport()) {
             responsiveVoice.speak("지도 리셋","Korean Female");
        }
},
	...
	news: function($scope) {
		...
		$('#news-div').load('https://newsapi.org/v2/top-headlines?country=kr&category=business&apiKey=5ff49d7d082b4810aa746f757c9fb588',function(){
